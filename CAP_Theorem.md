...
- **P — Partition Tolerance:** The system keeps running even if the network cable cuts or nodes can't talk to each other.
...
### Simple Bank Example (Delhi & Mumbai Branch)

Imagine my bank has 2 servers: **Delhi** and **Mumbai**. Both show I have **₹10,000**.

1. An inter-city network connection breaks between them $\rightarrow$ **Network Partition ($P$) happens!**
2. I deposit **₹5,000** in Delhi. Delhi balance = **₹15,000**. But Delhi cannot tell Mumbai because the connection is down.
3. Now a friend checks my balance in Mumbai. What should Mumbai do?
   - **Choice 1 (Consistency / CP):** Mumbai says _"Sorry, network is down, cannot check balance right now"_ (Error). Data stays safe, but **Availability is broken**.
   - **Choice 2 (Availability / AP):** Mumbai says _"You have ₹10,000"_ (old data). System answered successfully, but **Consistency is broken**.
...
- **Good for:** Money transactions, bank accounts, inventory systems where selling the same item twice is a disaster.
- **Examples:** Google Spanner, MongoDB (with majority write concern), HBase, ZooKeeper, etcd.

### AP (Availability + Partition Tolerance)

- Keeps answering fast using whatever local data it has, even if it might be slightly outdated. Nodes will sync later when the network fixes.
...
- Never tell the interviewer you will build a "CA" distributed database. CA only works on a single node.
